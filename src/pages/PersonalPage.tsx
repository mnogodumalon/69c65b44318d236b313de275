...
export default function PersonalPage() {
  const [records, setRecords] = useState<Personal[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Personal | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Personal | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Personal | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getPersonal());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Personal['fields']) {
    await LivingAppsService.createPersonalEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Personal['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updatePersonalEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deletePersonalEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Personal"
      subtitle={`${records.length} Personal im System`}
      action={
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('dienstgrad')}>
                <span className="inline-flex items-center gap-1">
                  Dienstgrad
                  {sortKey === 'dienstgrad' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('email_personal')}>
                <span className="inline-flex items-center gap-1">
                  E-Mail
                  {sortKey === 'email_personal' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{record.fields.dienstgrad?.label ?? '—'}</span></TableCell>
                <TableCell>{record.fields.einheit ?? '—'}</TableCell>
                <TableCell>{record.fields.telefon ?? '—'}</TableCell>
                <TableCell>{record.fields.email_personal ?? '—'}</TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={8} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Personal. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
