...
interface PersonalDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Personal['fields']) => Promise<void>;
  defaultValues?: Personal['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function PersonalDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = false, enablePhotoLocation = true }: PersonalDialogProps) {
  const [fields, setFields] = useState<Partial<Personal['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'personal');
      await onSubmit(clean as Personal['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "nachname": string | null, // Nachname\n  "personalnummer": string | null, // Personalnummer\n  "dienstgrad": LookupValue | null, // Dienstgrad (select one key: "oberfeldwebel" | "hauptfeldwebel" | "soldat" | "gefreiter" | "obergefreiter" | "hauptgefreiter" | "stabsgefreiter" | "unteroffizier" | "stabsunteroffizier" | "feldwebel" | "stabsfeldwebel" | "oberstabsfeldwebel" | "leutnant" | "oberleutnant" | "hauptmann" | "major" | "oberstleutnant" | "oberst" | "zivilangestellter") mapping: oberfeldwebel=Oberfeldwebel, hauptfeldwebel=Hauptfeldwebel, soldat=Soldat, gefreiter=Gefreiter, obergefreiter=Obergefreiter, hauptgefreiter=Hauptgefreiter, stabsgefreiter=Stabsgefreiter, unteroffizier=Unteroffizier, stabsunteroffizier=Stabsunteroffizier, feldwebel=Feldwebel, stabsfeldwebel=Stabsfeldwebel, oberstabsfeldwebel=Oberstabsfeldwebel, leutnant=Leutnant, oberleutnant=Oberleutnant, hauptmann=Hauptmann, major=Major, oberstleutnant=Oberstleutnant, oberst=Oberst, zivilangestellter=Zivilangestellter\n  "einheit": string | null, // Einheit / Abteilung\n  "telefon": string | null, // Telefon\n  "email_personal": string | null, // E-Mail\n  "vorname": string | null, // Vorname\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Personal['fields']>;
      });
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Personal bearbeiten' : 'Personal hinzufügen';
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="dienstgrad">Dienstgrad</Label>
            <Select
              value={lookupKey(fields.dienstgrad) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, dienstgrad: v === 'none' ? undefined : v as any }))}
            >
              <SelectTrigger id="dienstgrad"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
                <SelectItem value="oberfeldwebel">Oberfeldwebel</SelectItem>
                <SelectItem value="hauptfeldwebel">Hauptfeldwebel</SelectItem>
                <SelectItem value="soldat">Soldat</SelectItem>
                ...
                <SelectItem value="zivilangestellter">Zivilangestellter</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
            <Label htmlFor="einheit">Einheit / Abteilung</Label>
            <Input
              id="einheit"
          ...
          <div className="space-y-2">
            <Label htmlFor="email_personal">E-Mail</Label>
            <Input
              id="email_personal"
              type="email"
              value={fields.email_personal ?? ''}
              onChange={e => setFields(f => ({ ...f, email_personal: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
